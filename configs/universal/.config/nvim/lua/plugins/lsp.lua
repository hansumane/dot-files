return {
  ...
  config = function()
    ...
    local gen_no_lsp_syntax_hl = function(default_on_attach)
    ...
    local wanted_servers = {
      ...
      gopls = { enabled = false, config = nil },
      clangd = {
        enabled = false,
        config = {
          on_attach = gen_no_lsp_syntax_hl(vim.lsp.config['clangd'].on_attach)
        }
      },
      lua_ls = {
        enabled = false,
      ...
      rust_analyzer = {
        enabled = false,
        config = {
          on_attach = gen_no_lsp_syntax_hl(vim.lsp.config['rust_analyzer'].on_attach),
          settings = {
            ['rust-analyzer'] = {
    ...
    if not vars.listchars.hide_diagnostics then
    ...
    if vars.inlay_hints then
      if not vars.listchars.hide_inlay_hints then
