...
funcs.update_cc = function(info, new_cc, back)
  if not new_cc then
    if back then
      new_cc = vars.cc_dict[vim.o.colorcolumn][1] or vars.cd_init
    else
      new_cc = vars.cc_dict[vim.o.colorcolumn][2] or vars.cd_init
    end
  end
...
funcs.switch_listchars = function()
  if not vim.opt.listchars:get().space then
    if vars.inlay_hints and vars.listchars.hide_inlay_hints then
      vim.lsp.inlay_hint.enable(true)
    end
    if vars.listchars.hide_diagnostics then
...
-- currently Snacks.bufdelete is used instead
