...
local autocmds = {
  ...
  {
    -- - It is not generally required, but due to bug in neo-tree or Telescope,
    --   when the Telescope search result is in the current buffer, and neo-tree is open,
    ...
    -- - Also was used to disable colorcolumn in `neo-tree` buffers, and
    --   now is used to disable colorcolumn in `oil` buffers.
    name = 'BufWinEnter',
    condition = true,
    data = {
      callback = function()
        ...
        if vars.no_cc_ft[vim.bo.filetype] then
          vim.opt.colorcolumn = {}
        end
      end
    }
