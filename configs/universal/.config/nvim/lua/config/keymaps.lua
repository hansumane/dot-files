...
local maps = {
  ...
  { mods = 'n', map = '*', cmd = '*N',
    ...
    --== weird ==--
  { mods = { 'i', 'c' }, map = '<C-Space>', cmd = '<C-6>',
    opts = { silent = true, noremap = true, desc = 'Switch Language' } },
  ...
  { mods = 'n', map = '<leader>j', cmd = ':nohlsearch<CR>',
    ...
    --== specific ==--
  { mods = { 'n', 'v' }, map = '<leader>x', cmd = funcs.switch_listchars,
  ...
  { mods = 'n', map = '<C-t>', cmd = ':term<CR>',
    ...
    --== specific ==--
  { mods = { 'n', 'v', 'i' }, map = '<C-k>', cmd = function() funcs.update_cc('-> ') end,
    opts = { desc = 'Switch colorcolumn' } },

    --== specific ==--
  { mods = { 'n', 'v', 'i' }, map = '<C-j>', cmd = function() funcs.update_cc('<- ', nil, true) end,
    opts = { desc = 'Switch colorcolumn back' } },
...
local unmaps = {
  { mods = 'n', map = 'gri' }, -- <= v0.11
  { mods = 'n', map = 'grn' }, -- <= v0.11
  { mods = 'n', map = 'grr' }, -- <= v0.11
  { mods = 'n', map = 'grt' }, -- <= v0.11
  { mods = 'n', map = 'grx' }, -- >= v0.12
  { mods = { 'n', 'x' }, map = 'gra' },
}
