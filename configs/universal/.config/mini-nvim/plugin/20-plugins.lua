...
now(function()
  ...
  vim.g.rooter_patterns = {
    '.vi_project_root', '.git', '.luarc.json', 'compile_flags.txt', 'Cargo.toml'
  }
end)
...
later(function()
  ...
  local linters = {
    ...
    checkpatch = {
      ...
      args_force = true,
      args = {
        '--no-tree', '--no-signoff', '--show-types', '--strict', '--terse', '--file',
        '--ignore', table.concat({
          'SPDX_LICENSE_TAG', 'FILE_PATH_CHANGES', 'COMMIT_MESSAGE', 'PREFER_KERNEL_TYPES'
        }, ',')
      },
...
later(function()
  ...
  add {
    source = 'Saghen/blink.cmp', monitor = 'main', checkout = 'v1.6.0',
