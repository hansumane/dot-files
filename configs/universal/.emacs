...
(use-package corfu-terminal :ensure t
  :after corfu
  :config (if (not (display-graphic-p))
...
(use-package project :ensure t
  ...
  (defvar my/project-root-markers
    '(".project_root" ".helix" ".zed" ".git"
      ".luarc.json" "compile_flags.txt" "pyrightconfig.json"))
...
(use-package compile :ensure nil
  :custom (compile-command "./makedo.sh"))
...
(use-package markdown-mode :ensure t
  :config (setq markdown-regex-italic "$^"
...
(setq treesit-font-lock-level 4
      treesit-language-source-alist ;; <M-x> treesit-install-language-grammar <CR> {lang}
        '((c      "https://github.com/hansumane/tree-sitter-c")
...
;; because some *-ts-mode[s] work badly
(use-package tree-sitter-langs ;; :ensure t)
  :load-path "~/Downloads/manually/tree-sitter-langs"
  :init (setq tree-sitter-langs--testing t)) ;; prevents from downloading .so bundle
(use-package tree-sitter :ensure t
  :after tree-sitter-langs
  :hook ((c-mode c++-mode lua-mode sh-mode dts-mode perl-mode go-mode)
         . tree-sitter-mode)
  :config (add-hook 'tree-sitter-after-on-hook
            (lambda ()
              (font-lock-ensure)
              (tree-sitter-hl-mode))))
...
(use-package emacs
  :bind (("C-c k" . whitespace-mode)
         ("C-c j" . display-fill-column-indicator-mode))
